...
exports.listProsesPengiriman = ({ uid, limit, offset, cari }) => {
  let param = [limit ? limit : 50, offset ? offset : 0];

  return connection
    .query(
      `
      select * from proses_pengiriman
      ${
...
exports.tambahProsesPengiriman = (
  data = {
    nama_proses,
    keterangan,
    username,
  ...
  return connection
    .query(
      `
        insert into proses_pengiriman(${field}) values(${param}) returning *`,
...
exports.editProsesPengiriman = ({ uid, data }) => {
  ...
  return connection
    .query(
      `update proses_pengiriman set ${varSql} where uid = $1 returning *`,
      [uid, ...varData]
    )
    .then(({ rows }) => rows[0]);
};
...
exports.hapusProsesPengiriman = ({ uid }) => {
  return connection
    .query(`delete from proses_pengiriman where uid = $1 returning * `, [uid])
    .then(({ rows }) => rows);
};
